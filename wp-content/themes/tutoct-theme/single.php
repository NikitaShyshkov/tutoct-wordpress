...
get_header();
?>
<main class="bg-[#F6EEFF] w-full flex flex-col items-center relative">
    <!-- Изображение поста -->
    <?php if (has_post_thumbnail()) : ?>
        <div id="thumbnail-img" class="w-full h-[400px]">
            <img class="w-full h-full object-cover" src="<?php echo esc_url(get_the_post_thumbnail_url()); ?>" alt="<?php the_title(); ?>" />
        </div>
    <?php endif; ?>
    <a href="/blog" id="button-back" class="w-12 h-12 p-2 absolute left-[150px] top-[42px] bg-white rounded-[40px] outline outline-1 outline-offset-[-1px] outline-[#d7d7d7] inline-flex justify-start items-center gap-2.5">
    ...
    <div id="single-content" class="max-w-[1000px] w-full py-10 flex flex-col items-center">
        <?php
        if (have_posts()) :
        ...
        ?>
                ...
                <div class="comments-section flex flex-col">
                    <h2 class="text-[#120c38] text-2xl font-bold font-['Mulish'] mb-5">Залиште коментар</h2>
                    <?php
